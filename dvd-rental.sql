...
values(1,'Nahom','Bekele','nahom12@gmail.com',610,'true','now()','now()',1);
--To check the new customer information using
SELECT *
FROM customer
WHERE customer_id =616
--2. Update Customer Email: Update the email address of an existing customer by their customer ID.
--To update the email address of an existing customer
UPDATE customer
...
--To check if the rental duration of 'Academy Dinosaur'is updated
SELECT *
FROM film
WHERE title='Academy Dinosaur'
...
SELECT f.film_id,f.title ,SUM(amount) AS total_revenue
FROM film f
...
FROM rental
--To add the 'time_since_last_rental' column with age function.
...
JOIN rental r ON pa.rental_id =r.rental_id
...
WHERE fa.actor_id IS NULL; (edited) 